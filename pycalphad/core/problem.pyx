...
cdef class Problem:
    ...
    def chemical_potential_parameter_gradient(self, x_in):
        ...
        for compset in self.composition_sets:
            ...
            compset.phase_record.parameter_obj_grad(obj_grad_tmp, x)

            for dof_x_idx in range(num_statevars):
                for idx in range(num_parameters):
                    energy_parameter_prime[dof_x_idx, idx] += phase_amt * param_grad_tmp_view[dof_x_idx, idx]
            for dof_x_idx in range(compset.phase_record.phase_dof):
                for idx in range(num_parameters):
        ...
        mu_parameter_prime = np.dot(jac_pinv, energy_parameter_prime)
        return mu_parameter_prime[-len(self.nonvacant_elements):, :]
